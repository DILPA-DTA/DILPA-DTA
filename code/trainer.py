...
from sklearn.metrics import roc_auc_score, average_precision_score, roc_curve, confusion_matrix, precision_recall_curve, \
    precision_score
from models import binary_cross_entropy, cross_entropy_logits,concordance_index,mean_reversion_coefficient,get_rm2
...
class Trainer(object):
    def __init__(self, model, optim, device, train_dataloader, val_dataloader, test_dataloader, data_name, split, scheduler,
                 **config):
        self.model = model
        self.optim = optim
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            auroc, auprc, f1, sensitivity, specificity, accuracy, test_loss, thred_optim, precision, mse, ci, r2m = self.test(
                dataloader="test")
            test_lst = ["epoch " + str(self.best_epoch)] + list(
                map(float2str, [auroc, auprc, f1, sensitivity, specificity,
            ...
            #       str(specificity) + " Accuracy " + str(accuracy) + " Thred_optim " + str(thred_optim) + " mse " + str(mse) + " ci " + str(ci) + " r2m " + str(r2m) + " r2 " + str(r2))
            # print('Test at Best Model of Epoch '  + " mse " + str(mse) + " ci " + str(ci) + " r2m " + str(r2m) + " r2 " + str(r2) + " r2m_ " + str(r2m_))
            print('Test at Best Model of Epoch ' + " mse " + str(mse) + " ci " + str(ci) + " r2m " + str(r2m) )
            ...
            self.test_metrics["r2m"] = r2m
            # self.test_metrics["r2"] = r2
            # self.test_metrics["r2m_"] = r2m_
            # self.test_metrics["pearsoner"] = pearsonr
    ...
    def train_epoch(self):
        ...
        for i, (v_d, v_p, labels) in enumerate(tqdm(self.train_dataloader)):
            ...
            # print(type(v_p))
            self.step += 1
            v_d, v_p, labels = v_d.to(self.device), v_p.to(self.device), labels.float().to(
                self.device)
    ...
    def test(self, dataloader="test"):
        ...
        with torch.no_grad():
            ...
            for i, (v_d, v_p, labels) in enumerate(data_loader):
                v_d, v_p, labels = v_d.to(self.device), v_p.to(
        ...
        r2m = mean_reversion_coefficient(y_label, y_pred)
        if dataloader == "test":
            ...
            return auroc, auprc, np.max(f1[5:]), sensitivity, specificity, accuracy, test_loss, thred_optim, precision1, mse, ci, r2m
        else:
